    ...
    public UserAuthenticator(String credentialsPath) {
        ...
        try {
            if (!f.exists()) {
                ...
                if (!created) {
                    System.err.println("Impossibile creare il file delle credenziali: " + credentialsPath);
                }
            }
        } catch (IOException e) {
            throw new UncheckedIOException("Errore creando il file delle credenziali", e);
        }
    }
